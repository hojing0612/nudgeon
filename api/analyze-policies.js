...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rows = await db(`resources?select=id,title,summary,details,support_details,organization_name,application_url,reference_url,application_method,application_status,always_open,region_codes,raw_data&status=eq.published&or=${stale}&limit=20&order=verified_at.desc.nullslast`);
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        const result = await callClaudeTool({
          ...
          input:{ analysis_version:ANALYSIS_VERSION, today:new Date().toISOString().slice(0,10), policies:rows },
          maxTokens:8000
        });
