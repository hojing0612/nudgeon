...
test('추천 정책의 화면용 요약·조건·서류·비용표를 구조화해 저장한다', async () => {
  ...
  let getCount=0, patched=null, anthropicBody=null;
  global.fetch=async (url,options={})=>{
    if(String(url).includes('api.anthropic.com')){
      ...
      return {ok:true,json:async()=>({content:[{type:'tool_use',name:'save_policy_analysis',input:{analyses:[{
        id:'policy-1',category:'counseling',benefit_type:'counseling',benefit_summary:'전문 심리상담 8회',practical_value:9,
    ...
    if(options.method==='PATCH'){
      ...
      return {ok:true,text:async()=>JSON.stringify([patched])};
    }
    getCount+=1;
    if(getCount===1)return {ok:true,text:async()=>JSON.stringify([])};
    return {ok:true,text:async()=>JSON.stringify([{id:'policy-1',title:'심리상담 바우처',summary:'상담 지원',details:'지원 조건',support_details:'총 8회',required_documents:'신청서',raw_data:{}}])};
  };
  try{
    const res=responseRecorder();
    await handler({method:'POST',headers:{authorization:'Bearer cron-key'}},res);
    assert.equal(res.statusCode,200);
    assert.equal(res.body.analyzed,1);
    assert.equal(anthropicBody.messages[0].content.includes('presentation_version'),true);
