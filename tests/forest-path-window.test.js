...
test('창문은 1~5단계에 따라 커튼이 열리고 4·5단계 풍경이 이어진다', () => {
...
test('모든 단계는 세로 타원을 쓰고 3~5단계 커튼 진입 애니메이션을 제공한다', () => {
  assert.match(publicStyles, /\.window-glow\{[^}]*width:300px;height:360px;border-radius:50%/);
  assert.match(publicStyles, /data-window-screen="record"\] \.window-glow\{left:-25px;top:-35px;width:310px;height:370px/);
