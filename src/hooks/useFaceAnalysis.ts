...
export function useFaceAnalysis(active: boolean, cameraOn: boolean, videoRef: React.RefObject<HTMLVideoElement>) {
  const [metrics, setMetrics] = useState<FaceMetrics>({ gazeFocus: 0, presence: false, faceDetected: false });
  ...
  useEffect(() => {
    if (!active || !cameraOn) {
      stop();
      if (!active) setMetrics({ gazeFocus: 0, presence: false, faceDetected: false });
      return;
    }
      ...
      try {
        ...
        for (let y = 0; y < sampleSize; y++) {
          for (let x = 0; x < sampleSize; x++) {
            ...
            if (dist < sampleSize * 0.25) centerLum += lum;
            else edgeLum += lum;
        ...
        const faceDetected = avgLum > 35 && brightPixels / (sampleSize * sampleSize) > 0.25;
        const presence = faceDetected && avgLum > 40;
        const centerDominance = centerAvg > edgeAvg * 1.05;
        const stableBrightness = centerShift < 25;

        if (faceDetected) {
          frameCount++;
          if (centerDominance && stableBrightness) focusAccum = Math.min(frameCount, focusAccum + 1);
        } else {
        ...
        setMetrics({
          gazeFocus: faceDetected ? Math.max(35, Math.min(98, focusRatio)) : 0,
          presence,
          faceDetected,
        });
      } catch {
